...
!     The subroutine UTAE10 is from SRCC MSU numerical analysis library, file UTAE10.FOR
      ...
      RETURN
  100 PRINT 101
  101 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEG1C(AEG1P):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 1
  110 PRINT 111
  111 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEH1C(AEH1P):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 2
  120 PRINT 121
  121 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEG1R(AEG1D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 1
  130 PRINT 131
  131 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEH1R(AEH1D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 2
  140 PRINT 141
  141 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEE1R(AEE1D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 2
  150 PRINT 151
  151 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEJ1R(AEJ1D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 3
  300 PRINT 301
  301 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEG2C(AEG2P):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 6
  310 PRINT 311
  311 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEH2C(AEH2P):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 7
  320 PRINT 321
  321 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEG2R(AEG2D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 6
  330 PRINT 331
  331 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEH2R(AEH2D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 7
  340 PRINT 341
  341 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEE2R(AEE2D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 7
    1 PRINT 11
   11 FORMAT(' N1 - „‹Ÿ B›—ˆC‹EHˆŸ COCTBEHHOƒO ‡HA—EHˆŸ C ˆH„EKCOM ',
     *'IERR TPEYETCŸ O‹EE'/'     30 ˆTEPA–ˆ‰. PABˆ‹œHO B›—ˆC‹EH› ',
     *'COCTBEHH›E ‡HA—EHˆŸ C ˆH„EKCAMˆ'/'     IERR+1,IERR+2,...,N,',
     *' HO COCTBEHH›E BEKTOP› HE B›—ˆC‹ŸžTCŸ.')
      RETURN
    2 PRINT 12
   12 FORMAT(' N1 - „‹Ÿ B›—ˆC‹EHˆŸ COCTBEHHOƒO ‡HA—EHˆŸ C ˆH„EKCOM ',
     *'IERR TPEYETCŸ O‹EE'/'     30 ˆTEPA–ˆ‰. PABˆ‹œHO B›—ˆC‹EH› ',
     *'COCTBEHH›E ‡HA—EHˆŸ ˆ COCTBEHH›E'/'     BEKTOP› C ˆH„EKCAMˆ ',
     *'1,2,...,IERR-1, HO COCTBEHH›E ‡HA—EHˆŸ'/'     HEYOPŸ„O—EH›.')
      RETURN
    3 IF(IERR.GT.N.AND.IERR.LE.2*N) GO TO 4
      IF(IERR.GT.2*N) GO TO 5
      IF(IERR.LE.N) GO TO 2
    4 PRINT 14
   14 FORMAT(' N1 - HE BCE OAPH›E POˆ‡BE„EHˆŸ COOTBETCBYž™ˆX ',
     *'‹EMEHTOB OO—H›X'/'     „ˆAƒOHA‹E‰ HEOTPˆ–ATE‹œH›.')
      RETURN
    5 PRINT 15
   15 FORMAT(' N1 - ˆMEETCŸ PABHOE HY‹ž POˆ‡BE„EHˆE COOTBETCTBYž™ˆX ',
     *'‹EMEHTOB OO—H›X'/'     „ˆAƒOHA‹E‰, Pˆ—EM COMHO†ˆTE‹ˆ ',
     *'PABH› HY‹ž HEO„HOBPEMEHHO.B TOM'/'     C‹Y—AE HE CY™ECTBYET ',
     *'CˆMMETPˆ‡A–ˆˆ, HEOXO„ˆMO‰ „‹Ÿ PABˆ‹œHOƒO'/'     B›—ˆC‹EHˆŸ ',
     *'COCTBEHH›X BEKTOPOB.')
      RETURN
    6 PRINT 16
   16 FORMAT(' N1 - „‹Ÿ B›—ˆC‹EHˆŸ COCTBEHHOƒO ‡HA—EHˆŸ C ˆH„EKCOM ',
     *'IERR TPEYETCŸ O‹EE'/'     30 ˆTEPA–ˆ‰. PABˆ‹œHO B›—ˆC‹EH› ',
     *'COCTBEHH›E ‡HA—EHˆŸ C ˆH„EKCAMˆ'/'     IERR+1,IERR+2,...,N.')
      RETURN
    7 PRINT 17
   17 FORMAT(' N1 - „‹Ÿ B›—ˆC‹EHˆŸ COCTBEHHOƒO ‡HA—EHˆŸ C ˆH„EKCOM ',
     *'IERR TPEYETCŸ O‹EE'/'     30 ˆTEPA–ˆ‰. PABˆ‹œHO B›—ˆC‹EH› ˆ ',
     *'YOPŸ„O—EH› COCTBEHH›E ‡HA—EHˆŸ'/'    C ˆH„EKCAMˆ 1,2,...,'
     *'IERR-1. OHˆ HE OŸ‡ATE‹œHO ŸB‹ŸžTCŸ CAM›Mˆ'/'     MEHœ˜ˆMˆ ˆ‡ ',
     *'‚‘…• N COCTBEHH›X ‡HA—EHˆ‰.')
      RETURN
  350 IF(IERR.LT.0) GO TO 351
      GO TO 353
  351 PRINT 352
  352 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEJ2R(AEJ2D):',
     *'HE”ATA‹œHAŸ O˜ˆKA.'/' N1 - ˆMEETCŸ O„HO ˆ‹ˆ HECKO‹œKO PABH›X ',
     *'HY‹ž POˆ‡BE„EHˆ‰ COOTBETCTBYž™ˆX'/'     ‹EMEHTOB OO—H›X ',
     *'„ˆAƒOHA‹E‰, Pˆ—EM COMHO†ˆTE‹ˆ PABH› HY‹ž',
     */'     …Ž„Ž‚…Œ…Ž. ',
     *'B TOM C‹Y—AE BCE COCTBEHH›E ‡HA—EHˆŸ ',
     *'B›—ˆC‹EH› PABˆ‹œHO.')
      RETURN
  353 PRINT 354
  354 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEJ2R(AEJ2D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      IF(IERR.GT.N)GOTO 4
      IF(IERR.GT.0.AND.IERR.LE.N) GO TO 7
  180 PRINT 181
  181 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEH3C(AEH3P):',
     *'”ATA‹œHAŸ O˜ˆKA')
      IF(IERR.GT.3*N) GO TO 8
    9 PRINT 19
   19 FORMAT(' N1 - „‹Ÿ B›—ˆC‹EHˆŸ COCTBEHHOƒO BEKTOPA ',
     *'C ˆH„EKCOM IERR TPEYETCŸ O‹EE'/'     5 ˆTEPA–ˆ‰,',
     *' Pˆ TOM KOMOHEHT› TOƒO BEKTOPA'/'     PˆPABHˆBAžTCŸ HY‹ž.'
     *,
     *'EC‹ˆ TAKˆX COCTBEHH›X BEKTOPOB'/'      HECKO‹œKO, TO ',
     *'‡HA—EHˆE IERR O‹AƒAETCŸ PABH›M ˆH„EKCY'/'     OC‹E„HEƒO ',
     *'ˆ‡ ˆ•.')
      RETURN
    8 PRINT 18
   18 FORMAT(/'N1 - ‡HA—EHˆE MM MEHœ˜E ˆCTˆHHOƒO —ˆC‹A COCTBEHH›X ',
     *'‡HA—EHˆ‰ M HA ˆHTEPBA‹E.')
      RETURN
  190 PRINT 191
  191 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEH4C(AEH4P):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 8
  200 PRINT 201
  201 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEH3R(AEH3D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      IF(IERR.GT.3*N) GO TO 8
      GO TO 9
  210 PRINT 211
  211 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEH4R(AEH4D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      GO TO 8
  220 PRINT 221
  221 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEE9R(AEE9D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      IF(IERR.GT.3*N) GO TO 8
      GO TO 9
  230 PRINT 231
  231 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEJ3R(AEJ3D):',
     *'”ATA‹œHAŸ O˜ˆKA')
      ...
      GO TO 1111
   10 PRINT 20
   20 FORMAT(' N1 - ‡A„AHHAŸ MATPˆ–A HE ŸB‹ŸETCŸ MATPˆ–E‰ ŸKOˆ, ',
     *'T.K. ˆMEETCŸ PABHOE'/'     HY‹ž POˆ‡BE„EHˆE ',
     *'COOTBETCTBYž™ˆX ‹EMEHTOB OO—H›X'/'     „ˆAƒOHA‹E‰, Pˆ—EM ',
     *'COMHO†ˆTE‹ˆ PABH› HY‹ž HEO„HOBPEMEHHO.'/'     B TOM C‹Y—AE ',
     *'HET CˆMMETPˆ‡A–ˆˆ, HEOXO„ˆMO‰ „‹Ÿ PABˆ‹œHOƒO'/
     *'     ‚›—ˆ‘‹…ˆŸ COCTBEHH›X BEKTOPOB.')
      RETURN
 1111 PRINT 21
   21 FORMAT(' N1 - ‡A„AHHAŸ MATPˆ–A HE  ŸB‹ŸETCŸ MATPˆ–E‰ ŸKOˆ, ',
     *'TAK KAK HE BCE'/'     OAPH›E POˆ‡BE„EHˆŸ COOTBETCTBYž™ˆX ',
     *'‹EMEHTOB OO—H›X „ˆAƒOHA‹E‰'/'     HEOTPˆ–ATE‹œH›.')
      RETURN
  250 IF(IERR.LT.0) GO TO 251
      GO TO 254
  251 PRINT 252
  252 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEJ4R(AEJ4D):',
     *'HE”ATA‹œHAŸ O˜ˆKA')
      PRINT 253
  253 FORMAT(' N1 - ‡A„AHHAŸ MATPˆ–A HE ŸB‹ŸETCŸ MATPˆ–E‰ ŸKOˆ, ',
     *'TAK KAK ˆMEETCŸ PABHOE'/'     HY‹ž POˆ‡BE„EHˆE ',
     *'COOTBETCTBYž™ˆX ‹EMEHTOB OO—H›X'/'    „ˆAƒOHA‹E‰, Pˆ—EM ',
     *'COMHO†ˆTE‹ˆ PABH› HY‹ž HEO„HOBPEMEHHO.'/'     B TOM ',
     *'C‹Y—AE COCTBEHH›E ‡HA—EHˆŸ B›—ˆC‹EH› PABˆ‹œHO.')
      RETURN
  254 PRINT 255
  255 FORMAT(' ˆ‹ˆOTEKA HˆB– MƒY, O„POƒPAMMA AEJ4R(AEJ4D):',
     *'”ATA‹œHAŸ O˜ˆKA')
