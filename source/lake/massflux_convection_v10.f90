...
real(8),               INTENT(IN)   ::  PTSTEP       ! timestep
real(8), DIMENSION(:), INTENT(IN)   ::  PZZ          ! depths of flux levels (w points) (1:M)
real(8), DIMENSION(:), INTENT(IN)   ::  ZZM          ! depths of mass levels (T)        (1:M+1)
real(8), DIMENSION(:), INTENT(IN)   ::  DZF          ! distance between full levels     (1:M)
real(8),               INTENT(IN)   ::  PSDENS       ! surface density flux
real(8),               INTENT(IN)   ::  PSFT,PSLE,PSFSAL  ! surface sensible heat flux, latent heat flux and salinity flux
real(8),               INTENT(IN)   ::  PSFU,PSFV    ! normal surface fluxes of (u,v)

!    prognostic variables at t- deltat
real(8), DIMENSION(:), INTENT(IN) ::  PUM,PVM,PWM ! current components (1:M+1)
real(8), DIMENSION(:), INTENT(IN) ::  PTKEM       ! TKE                (1:M)
!
real(8), DIMENSION(:), INTENT(IN) ::  PUT,PVT,PWT ! Wind  at t         (1:M+1)
!
real(8), DIMENSION(:), INTENT(IN) ::  PDENSM       ! density at time t-1     (1:M+1)
real(8), DIMENSION(:), INTENT(IN) ::  PTM          ! temperature at time t-1 (1:M+1)
real(8), DIMENSION(:), INTENT(IN) ::  PSALM        ! salinity at time t-1    (1:M+1)

!
real(8), DIMENSION(:), INTENT(OUT)::  PEMF,    &    ! Mass Flux (M)          (1:M)
                                   PDENS_DOWN, &    ! DOWNdraft density      (1:M+1)
                                   PT_DOWN,    &    ! DOWNdraft temperature  (1:M+1)
                                   PSAL_DOWN        ! DOWNdraft salinity     (1:M+1)
real(8), INTENT(INOUT)            ::  ZINV
...
INTEGER,            INTENT(IN)   ::  KTCOUNT      ! number of the current timestep
real(8),               INTENT(IN)   ::  PTSTEP       ! timestep
real(8), DIMENSION(:), INTENT(IN)   ::  PZZ          ! height of flux levels (w points)
real(8), DIMENSION(:), INTENT(IN)   ::  ZZM          ! height of mass levels (T)
real(8), DIMENSION(:), INTENT(IN)   ::  DZF          ! distance between full levels
real(8),               INTENT(IN)   ::  PSDENS       ! Densisty surface flux
real(8),               INTENT(IN)   ::  PSFT,PSLE,PSFSAL  ! surface sensible heat fluxe, latent heat flux and salinity flux
real(8),               INTENT(IN)   ::  PSFU,PSFV    ! normal surface fluxes of (u,v) parallel to the orography

!    prognostic variables at t- deltat
real(8), DIMENSION(:), INTENT(IN) ::  PUM,PVM,PWM ! wind components
real(8), DIMENSION(:), INTENT(IN) ::  PTKEM       ! TKE
!
real(8), DIMENSION(:), INTENT(IN) ::  PUT,PVT,PWT ! Wind  at t
!
real(8), DIMENSION(:), INTENT(IN) ::  PDENSM       ! density at time t-1
real(8), DIMENSION(:), INTENT(IN) ::  PTM          ! temperature at time t-1
real(8), DIMENSION(:), INTENT(IN) ::  PSALM        ! salinity at time t-1

!
real(8), DIMENSION(:), INTENT(OUT)::  PEMF,       &    ! Mass Flux
...
real(8), parameter :: ZTKEMIN = 1.D-10

real(8) :: XWTHVSURF,ZDT,ZDDENS,ZDSAL
real(8) :: XCSTMF
real(8) :: ALPHA
real(8) :: XEPSCST
real(8) :: XABUO
real(8) :: XBEPS
real(8) :: XZS
real(8) :: ZVARW_F_MIN ! V. M. Stepanenko, 01.04.08
INTEGER :: ISTART
!----------------------------------------------------------------------------
!
!*      PRELIMINARIES
...
!
     ZDDENS = (PSDENS/SQRT(DMAX1(ZTKEM_F(ISTART),ZTKEMIN) ))*ALPHA
     ZDT    = (PSFT  /SQRT(DMAX1(ZTKEM_F(ISTART),ZTKEMIN) ))*ALPHA
     ZDSAL  = (PSFSAL/SQRT(DMAX1(ZTKEM_F(ISTART),ZTKEMIN) ))*ALPHA
!
!-------------------------------------------------
...
!
   DO JK=ISTART,IKE-1
      IF (ZZM(JK) < ZINV) THEN
         ZEPSILON(JK) = MAX(0.,MIN(XEPSCST/(PZZ(JK+1)-PZZ(JK)),                  &
...
!     Note, that the temperature, passing to WATER_DENS_TS, must be in Celsius degrees
      PDENS_DOWN(JK+1) = WATER_DENS_TS(dble(PT_DOWN(JK+1)),0.d0)
