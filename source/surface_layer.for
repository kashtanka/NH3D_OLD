      subroutine inisurf(fnmap)
	use alloc , only:
     :xsea,tsfix,z0,nx,ny
	implicit none
	character*80 fnmap,fext
	external fext
      open (1,file=fext(fnmap,'sea'),status='old')
      call readgrd(1,xsea,0,nx+1,0,ny+1)
      close(1)
	open (1,file=fext(fnmap,'tsf'),status='old')
      call readgrd(1,tsfix,0,nx+1,0,ny+1)
      close(1)
	...
	use alloc, only :
     :    nx,ny,ns,g,akapa,hsuf,phisuf,deltaz,
     :	u,v,pts,pt,qv,sigma0,pp,phis,phi,uvsuf,
     :    ptop,qif,phig,tsfix,z0,xsea,h,le,cdm,
     :    ust_s,tsurf,rhoa,nx1,ny1,dzits,tst_s,
     :    imp,qst_s,nstep,dt
	implicit none
	real,external:: qsat
	integer ix,iy,is
	real(8) qa,taixiy,p,hflux,Elatent,ustar,cdu,Ribulk,dzita,Tstar,ro
     :        ,z1,tau,qstar,water
	do iy=2,ny1-1
      do ix=2,nx1-1
...
c
        ...
        if (qif.gt.0) then
        qa=qv(ix,iy,ns-1,2)
        !write(0,*) qv(ix,iy,30,2)
        rhoa=p/(287.05*(taixiy*(1.+0.61*qa)))
        else
        qa=0.
        rhoa=p/(287.05*taixiy)
        endif
        taixiy=pts(ix,iy,ns-1)+pt(ix,iy,ns-1,3)
         
  
        	!write(0,*)ix,iy,xsea(ix,iy)
        call surf_scheme3(xsea(ix,iy),
     :       taixiy,tsurf(ix,iy),qa,
     :       uvsuf(ix,iy),p,deltaz(ix,iy),z0(ix,iy),
	...
	if(ustar*tstar.gt.0) qstar=0.
	if(ustar*tstar.gt.0) le(ix,iy)=0.
	cdm(ix,iy)=cdu
        ust_s(ix,iy)=ustar
	tst_s(ix,iy)=Tstar
	qst_s(ix,iy)=qstar
...
!----------------flux aggregation------------------------------------!	
	if (xsea(ix,iy).lt.1.and.xsea(ix,iy).gt.0) then
	tsurf(ix,iy)=271.35
	water=1
	...
	call surf_scheme3(water,
     ...
     :       cdu,hflux,Elatent,ustar,qif,Ribulk,dzita,Tstar,rhoa,z1
     :       ,tau,qstar)
	h(ix,iy)=xsea(ix,iy)*hflux+(1.-xsea(ix,iy))*h(ix,iy)
	imp(ix,iy)=xsea(ix,iy)*tau+(1.-xsea(ix,iy))*imp(ix,iy)
	if (qif.gt.0) then
	le(ix,iy)=xsea(ix,iy)*Elatent+(1.-xsea(ix,iy))*le(ix,iy)
	else
	le(ix,iy)=0.
	endif
	cdm(ix,iy)=xsea(ix,iy)*cdu+(1.-xsea(ix,iy))*cdm(ix,iy)
        ust_s(ix,iy)=xsea(ix,iy)*ustar+(1.-xsea(ix,iy))*ust_s(ix,iy)
	tst_s(ix,iy)=xsea(ix,iy)*Tstar+(1.-xsea(ix,iy))*tst_s(ix,iy)
	qst_s(ix,iy)=xsea(ix,iy)*qstar+(1.-xsea(ix,iy))*qst_s(ix,iy)
!	dzits(ix,iy)=xsea(ix,iy)*dzita+(1.-xsea(ix,iy))*dzits(ix,iy)
	endif
	call extrpp(nx1,ny1,imp,1,nx1,1,ny1)
	call extrpp(nx1,ny1,deltaz,1,nx1,1,ny1)
      call extrpp(nx1,ny1,h,1,nx1,1,ny1)
      call extrpp(nx1,ny1,le,1,nx1,1,ny1)
      call extrpp(nx1,ny1,cdm,1,nx1,1,ny1)
      call extrpp(nx1,ny1,ust_s,1,nx1,1,ny1)
      call extrpp(nx1,ny1,tst_s,1,nx1,1,ny1)
      call extrpp(nx1,ny1,qst_s,1,nx1,1,ny1)
      call extrpp(nx1,ny1,dzits,1,nx1,1,ny1)
  	enddo
	enddo
